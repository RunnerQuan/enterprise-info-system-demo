...
func GetGroupedEmployeeDepartments() ([]models.GroupedEmployeeDepartment, error) {
	...
	for rows.Next() {
		...
		// 使用存储过程获取部门数量
		var deptCount int
		if err := utils.DB.Raw("CALL CountDepartmentsByEmployee(?, @count)", grouped.EmpNo).Error; err != nil {
			return nil, err
		}
		if err := utils.DB.Raw("SELECT @count").Scan(&deptCount).Error; err != nil {
			return nil, err
		}
		...
		grouped.DepartmentCount = deptCount
...
// 获取部门内所有员工
func GetEmployeesInDepartment(deptNo int) ([]models.EmployeeInfo, error) {
