...
func DeleteDepartment(id int) error {
	return utils.DB.Transaction(func(tx *gorm.DB) error {
		...
		if err := tx.Model(&models.EmployeeDepartment{}).
			Where("DeptNo = ? AND EdStatus = 1", id).
...
func GetGroupedEmployeeDepartments() ([]models.GroupedEmployeeDepartment, error) {
	...
	rows, err := utils.DB.Raw(`
		...
		FROM Employees e
		INNER JOIN Employee_Department ed ON e.EmpNo = ed.EmpNo
	`).Rows()
	if err != nil {
		return nil, err
	}
	...
	for rows.Next() {
		...
		err := utils.DB.Raw(`
			SELECT ed.EdID, ed.DeptNo, d.DeptName, ed.EdEntryDate, ed.EdLeaveDate, ed.EdStatus
...
func AddEmployeeDepartment(req *models.EmployeeDepartmentRequest) error {
	...
	if err := utils.DB.First(&emp, req.EmpNo).Error; err != nil {
		return errors.New("员工不存在")
	}

	var dept models.Department
	if err := utils.DB.First(&dept, req.DeptNo).Error; err != nil {
		return errors.New("部门不存在")
	}
...
func UpdateEmployeeDepartment(edID int, req *models.EmployeeDepartmentRequest) error {
	...
	if err := utils.DB.First(&ed, edID).Error; err != nil {
	...
	return utils.DB.Transaction(func(tx *gorm.DB) error {
		// 如果状态从在职变为离职，需要更新部门人数
		if ed.EdStatus == 1 && req.EdStatus == 2 {
