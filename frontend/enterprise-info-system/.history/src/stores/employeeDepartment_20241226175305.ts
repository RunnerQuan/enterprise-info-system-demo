...
export const useEmployeeDepartmentStore = defineStore('employeeDepartment', () => {
  ...
  // 按员工分组的部门关系
  const groupedEmployeeDepartments = computed<GroupedEmployeeDepartment[]>(() => {
    const grouped = new Map<number, GroupedEmployeeDepartment>()
    
    employeeDepartmentDetails.value.forEach(ed => {
      if (!grouped.has(ed.empNo)) {
        const employee = employeeStore.employees.find(e => e.empNo === ed.empNo)
        grouped.set(ed.empNo, {
          empNo: ed.empNo,
          employeeName: employee ? `${employee.lastName}${employee.firstName}` : '未知',
          departments: []
        })
      }
      
      grouped.get(ed.empNo)?.departments.push({
        edID: ed.edID,
        deptNo: ed.deptNo,
        departmentName: ed.departmentName,
        edEntryDate: ed.edEntryDate,
        edLeaveDate: ed.edLeaveDate,
        edStatus: ed.edStatus
      })
    })
