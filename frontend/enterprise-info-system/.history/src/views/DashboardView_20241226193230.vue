...
const stats = computed(() => {
  ...
  return {
    ...
    activeEmployeeDepartments: employeeDepartmentStore.employeeDepartmentDetails?.reduce(
      (count, ed) => ed.edStatus === 1 ? count + 1 : count, 
      0
    ) ?? 0
  }
})

// 性别分布数据 - 直接使用 stats 中的数据
const genderDistribution = computed(() => [
  { name: '男', value: stats.value.maleEmployees },
  { name: '女', value: stats.value.femaleEmployees }
])
        ...
        <div class="bg-white rounded-lg shadow p-6">
          <div class="flex items-center">
            ...
            <div class="ml-4">
              ...
              <p class="mt-2 text-3xl font-bold text-gray-900">{{ stats.maleEmployees }}</p>
            </div>
          </div>
        </div>
        ...
        <div class="bg-white rounded-lg shadow p-6">
          <div class="flex items-center">
            ...
            <div class="ml-4">
              ...
              <p class="mt-2 text-3xl font-bold text-gray-900">{{ stats.femaleEmployees }}</p>
            </div>
          </div>
        </div>
      </div>
