...
onMounted(async () => {
  try {
    ...
    showError(error.response?.data?.error || '加载部门列表失败')
  }
})
...
const handleAdd = async (departmentData: Omit<Department, 'deptNo'>) => {
  try {
    ...
    showError(error.response?.data?.error || '添加部门失败')
  }
}
...
const handleEdit = async (department: Department) => {
  try {
    ...
    showError(error.response?.data?.error || '更新部门失败')
  }
}
...
const handleDelete = async (department: Department) => {
  ...
  try {
    ...
    showError(error.response?.data?.error || '删除部门失败')
  }
}
