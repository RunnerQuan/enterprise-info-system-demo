  ...
  <div class="space-y-4">
    ...
    <div class="bg-white shadow overflow-hidden sm:rounded-lg">
      <ul v-if="employeeDepartmentStore.employeeDepartmentDetails.length" class="divide-y divide-gray-200">
        <li v-for="ed in employeeDepartmentStore.employeeDepartmentDetails" :key="ed.edID" 
            class="p-4 hover:bg-gray-50 transition-colors duration-200">
          <div class="flex justify-between items-center">
            <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 flex-grow">
              <!-- 员工信息 -->
              <div class="flex items-center space-x-3">
                <div class="flex-shrink-0">
                  <div class="w-10 h-10 rounded-full bg-indigo-100 flex items-center justify-center">
                    <span class="text-indigo-700 font-medium text-sm">{{ ed.employeeName[0] }}</span>
                  </div>
                </div>
                <div>
                  <div class="text-sm font-medium text-gray-900">{{ ed.employeeName }}</div>
                  <div class="text-sm text-gray-500">员工</div>
                </div>
              </div>
              ...
              <div class="flex items-center">
                ...
                <div>
                  <div class="text-sm font-medium text-gray-900">{{ ed.departmentName }}</div>
              ...
              <!-- 入职日期 -->
              <div class="flex items-center">
                <div class="text-sm text-gray-900">{{ ed.edEntryDate }}</div>
                <div class="text-xs text-gray-500 ml-2">入职</div>
              </div>

              <!-- 离职日期 -->
              <div class="flex items-center">
                <div class="text-sm text-gray-900">{{ ed.edLeaveDate || '在职' }}</div>
                <div class="text-xs text-gray-500 ml-2">离职</div>
              </div>
            ...
            <div class="flex space-x-2 ml-4">
              <button
                ...
                class="inline-flex items-center px-3 py-1 border border-indigo-600 rounded text-sm font-medium text-indigo-600 hover:bg-indigo-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500"
              >
              ...
              <button
                ...
                class="inline-flex items-center px-3 py-1 border border-red-600 rounded text-sm font-medium text-red-600 hover:bg-red-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500"
              >
                <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" 
