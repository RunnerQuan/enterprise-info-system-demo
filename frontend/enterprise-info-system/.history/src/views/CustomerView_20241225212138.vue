  ...
  <div class="space-y-6">
    ...
    <div class="bg-white shadow overflow-hidden sm:rounded-lg">
      <ul v-if="customerStore.customers.length" class="divide-y divide-gray-200">
        <li v-for="customer in customerStore.customers" :key="customer.customerID" class="px-6 py-4">
          <div class="flex items-center justify-between">
            <div class="space-y-1">
              <!-- 客户姓名和公司 -->
              <div class="text-lg font-medium text-gray-900">
                {{ customer.customerName }}
                <span class="ml-2 text-sm text-gray-500">{{ customer.company }}</span>
              </div>
              <!-- 性别和年龄 -->
              <div class="text-sm text-gray-500">
                性别：{{ customer.sex }}
                <span class="ml-4">年龄：{{ customer.age }}</span>
              </div>
              <!-- 联系方式 -->
              <div class="text-sm text-gray-500">
