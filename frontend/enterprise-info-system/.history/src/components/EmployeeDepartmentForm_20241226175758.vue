...
const form = ref({
  ...
  edEntryDate: props.employeeDepartment?.edEntryDate ?? getCurrentDate(),
  edLeaveDate: props.employeeDepartment?.edLeaveDate ?? '',
  edStatus: props.employeeDepartment?.edStatus ?? (1 as 1 | 2)
})
...
const handleSubmit = () => {
  const employeeDepartmentData = {
    ...
    edStatus: form.value.edStatus as 1 | 2,
    edEntryDate: form.value.edEntryDate.replace(/\//g, '-'),
    edLeaveDate: form.value.edLeaveDate ? form.value.edLeaveDate.replace(/\//g, '-') : null
  }
