...
const emit = defineEmits<{
  submit: [employee: EmployeeRequest]
  cancel: []
}>()
...
const form = ref({
  ...
  hireDate: props.employee?.hireDate ? new Date(props.employee.hireDate).toISOString().split('T')[0] : new Date().toISOString().split('T')[0],
  birthday: props.employee?.birthday ? new Date(props.employee.birthday).toISOString().split('T')[0] : '',
...
const handleSubmit = () => {
  ...
  if (props.isEdit) {
    employeeData.empNo = props.employee!.empNo
  }
