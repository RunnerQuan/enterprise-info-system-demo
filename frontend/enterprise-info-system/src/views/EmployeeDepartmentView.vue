...
const currentEmployeeDepartment = ref<EmployeeDepartment>()

onMounted(async () => {
  try {
    await employeeDepartmentStore.loadEmployeeDepartments()
  } catch (error) {
    console.log('暂无员工部门关系数据')
  }
})
...
const handleAdd = async (employeeDepartmentData: Omit<EmployeeDepartment, 'edID'>) => {
  try {
    ...
    showError(typeof error === 'string' ? error : '添加员工部门关系失败')
  }
}
...
const openEditModal = (employeeDepartment: EmployeeDepartment) => {
  currentEmployeeDepartment.value = {
    ...employeeDepartment,
    edStatus: employeeDepartment.edStatus as 1 | 2
  }
    ...
    <div class="bg-white shadow overflow-hidden sm:rounded-lg">
                ...
                <div class="flex items-center">
                  ...
                  <div>
                    ...
                    <div class="text-sm text-gray-500">
                      {{ formatDate(dept.edEntryDate) }} ~ {{ formatDate(dept.edLeaveDate) }}
                    </div>
                  </div>
                </div>
      ...
      <div v-else class="py-8">
        <div class="text-center">
          ...
          <p class="mt-2 text-sm text-gray-500">当前企业员工和部门不存在关系</p>
          <p class="mt-1 text-sm text-gray-400">您可以点击上方的"添加关系"按钮来建立关联</p>
        </div>
      </div>
    </div>
